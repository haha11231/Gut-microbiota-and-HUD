...
library(ppcor)
data1 <- read.csv('cellmark.csv', header = T, row.names = 1)
data2 <- read.csv("phenotypep2.csv", row.names = 1, header = T)
...
data$cellmarker <- factor(data$cellmarker, levels = c("Weissella", "Fusicatenibacter", "Blautia", "Roseburia", "Romboutsia", "Lactobacillus", "Ruminococcus2", "Faecalibacterium", "Clostridium_XlVb", "Megamonas", "Streptococcus", "Anaerostipes", "Paraprevotella", "Butyricicoccus", "Actinomyces", "Dorea", "Megasphaera", "Terrisporobacter", "Turicibacter", "Clostridium_XlVa", "Escherichia", "Flavonifractor", "Intestinibacter", "Coprococcus"))


ggplot(data, aes(cellmarker, class))+
...
corrplot(corr = col, p.mat = col.p, method = "square", type = "full",
         ...
         # col = colorRampPalette(c("darkblue", "white", "#CC0000"))(100), #heroin
         # col = colorRampPalette(c("deepskyblue2", "white", "hotpink2"))(100), #withdrawal
